...
!function(G) {
  ...
  _.deep_pluck = _.deepPluck = function f(data, keys) {
    ...
    var keys = _.isString(keys) ? keys.split(/\s*\.\s*/) : [''], len = keys.length, new_keys; // keys 있는지 확인하고 있다면 split로 (.) 기준으로 배열을 만들어준다 -> ['posts','comments','user', 'name']
    return _.reduce(likearr(data) ? data : [data], function(mem, val) { // reduce 초기값으로 mem -> 배열로 정의 / val -> data 값 (객체 값으로 들어온다 ? 현재 {posts: Array(2), id: 101, name: 'ID'} )
      
      var current = val, i = -1; // current = val / i = -1
      while (++i < len && _.isObject(current) && !_.isArray(current)) current = current[keys[i]]; // 초기 ++i < len -> true 1 < 4 / 객체인지 확인하고 배열이 아닌지 확인한다
